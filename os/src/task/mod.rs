...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        task0.start_time = Some(get_time_ms()); // record start time
    ...
    fn get_current_start_time(&self) -> Option<usize> {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
...
pub fn get_current_task_time() -> usize {
    let start_time = TASK_MANAGER.get_current_start_time().unwrap();
